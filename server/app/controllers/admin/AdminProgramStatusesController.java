...
public final class AdminProgramStatusesController extends CiviFormController {
  ...
  public Result createOrUpdate(Http.Request request, long programId)
    ...
    if (!maybeSetDefault.isPresent() || maybeSetDefault.get().isBlank()) {
      setDefault = false;
    ...
    if (setDefault
        && previousDefaultStatus.isPresent()
        && !previousDefaultStatus.get().statusText().equals(form.get().getConfiguredStatusText())) {
      toastMessage = form.get().getStatusText() + " has been updated to the default status";
    }
  ...
  private ErrorAnd<ProgramDefinition, CiviFormError> createOrEditStatusFromFormData(
    ...
    if (formData.getConfiguredStatusText().isEmpty()) {
      ...
      newStatusBuilder = newStatusBuilder.setDefaultStatus(Optional.of(setDefault));
